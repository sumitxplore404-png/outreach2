...
if ($batchResponse.StatusCode -eq 200) {
    ...
    if ($recentBatch) {
        ...
        if ($generateResponse.StatusCode -eq 200) {
            ...
            $sendResponse = Invoke-WebRequest -Uri "http://localhost:3000/api/batch/send" -Method POST -Headers @{ "Content-Type" = "application/json" } -Body "{ `"batchId`": `"$batchId`", `"generatedEmails`": $($generateResult.generatedEmails | ConvertTo-Json) }" -WebSession $session
